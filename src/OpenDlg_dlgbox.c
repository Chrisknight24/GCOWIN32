...
BOOL OpenDlg_dialogbox(int iType,HWND hwndOwner,HINSTANCE hinst)
{
    ...
    *lpw++ = 0; // No creation data
    ...
    WS_TABSTOP;
    //Cas ou User souhaite creer le main Window
    if(isParentWnd && !isTypeModDlg)
        lpdit->style|=WS_DISABLED;
    ...
    *lpw++ = 0; // No creation data
    ...
    lpdit = AlignLpdit(lpw);
    lpdit->cx = (strlen(name)+1)*cxChar/2; lpdit->cy = cyChar;
    lpdit->x =rcDlg.right/2+rcDlg.right/4 -4*cxChar ; lpdit->y =9*cxChar;
    lpdit->id = ID_ADD_MENU; // ADD MENU button identifier
    lpdit->style = WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON|
    WS_TABSTOP;
    ...
    *lpw++ = 0; // No creation data
...

    {
        ...
        DialogBoxIndirectParamA(hinst,(LPDLGTEMPLATE)hgbl,
