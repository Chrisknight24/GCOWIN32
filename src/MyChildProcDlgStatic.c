/*Nom de File:MyChildProcDlgStatic.c
Role:File qui gere l les fenetres childs personnalisees de la boite
de dialogue DLG_STATIC
BY CHIRSTIAN EDOUARDO LE 17/09/2024 a 15h56*/
...
LRESULT CALLBACK MyChildProcDlgStatic(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
    //Cette procedure de fenetre est sensee gerer tous les childs present
    //dans la boite de dialogue de DLG_STATIC, mais uniquement ceux crees avec la classe personnalisee
    ...
    int idChildDlgStatic;
    idChildDlgStatic=GetDlgCtrlID(hwnd);

    idDlg=DLG_STATIC;

    if(idChildDlgStatic==ID_STYLE_LIST)
    {
        FILE *File=NULL;
...
(HINSTANCE)GetWindowLongPtr(hwnd,GWLP_HINSTANCE),NULL);
            ...
            while(fgets(buf,1024,File))
            {
                ...
                szName=strtok(buf," ");
                if( ((c=strstr(buf,"WS_")) || (c=strstr(buf,"SS_"))) && c==buf)//SS car on traite le static
                {
                    if(i==1) dwStyleChild=dwStyleChild | WS_GROUP|WS_TABSTOP;
                    else if(i==2) dwStyleChild=dwStyleChild & ~WS_GROUP;
                    //Si  buf est valide(commence par SS ou par WS)
    ...
    }//fin if idChildDlgStatic==ID_STYLE_LIST


    //Procedure du ID_ASPECT
    else if(idChildDlgStatic==ID_ASPECT)
    {
    ...
    }//fin  if(idChildDlgStatic==ID_ASPECT)
    else if(idChildDlgStatic==ID_DESCRIPTION)
    {
