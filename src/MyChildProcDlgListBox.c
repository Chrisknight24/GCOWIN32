/*Nom de File:MyChildProcDlgListBox.c
Role:File qui gere l les fenetres childs personnalisees de la boite
de dialogue DLG_LISTBOX
BY CHIRSTIAN EDOUARDO LE 13/09/2024 a 13h57*/
...
LRESULT CALLBACK MyChildProcDlgListBox(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
    //Cette procedure de fenetre est sensee gerer tous les childs present
    //dans la boite de dialogue de DLG_LISTBOX, mais uniquement ceux crees avec la classe personnalisee
    ...
    int idChildDlgListBox;
    idDlg=DLG_LISTBOX;

    idChildDlgListBox=GetDlgCtrlID(hwnd);

    if(idChildDlgListBox==ID_STYLE_LIST)
    {
        FILE *File=NULL;
...
(HINSTANCE)GetWindowLongPtr(hwnd,GWLP_HINSTANCE),NULL);
            ...
            while(fgets(buf,1024,File))
            {
                ...
                szName=strtok(buf," ");
                if( ((c=strstr(buf,"WS_")) || (c=strstr(buf,"LBS_"))) && c==buf)//LBS car on traite les listbox
                {
                    if(i==1) dwStyleChild=dwStyleChild | WS_GROUP|WS_TABSTOP;
                    else if(i==2) dwStyleChild=dwStyleChild & ~WS_GROUP;
                    //Si  buf est valide(commence par LBS ou par WS)
    ...
    }//fin if idChildDlgListBox==ID_STYLE_LIST


    //Procedure du ID_ASPECT
    else if(idChildDlgListBox==ID_ASPECT)
    {
    ...
    }//fin  if(idChildDlgListBox==ID_ASPECT)
    else if(idChildDlgListBox==ID_DESCRIPTION)
    {
