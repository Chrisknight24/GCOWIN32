/*Nom de File:MyChildProcDlgEditBox.c
Role:File qui gere l les fenetres childs personnalisees de la boite
de dialogue DLG_EDITBOX
BY CHIRSTIAN EDOUARDO LE 13/09/2024 a 14h46*/
...
LRESULT CALLBACK MyChildProcDlgEditBox(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
    //Cette procedure de fenetre est sensee gerer tous les childs present
    //dans la boite de dialogue de DLG_EDITBOX, mais uniquement ceux crees avec la classe personnalisee
    ...
    int idChildDlgEditBox;
    idDlg=DLG_EDITBOX;

    idChildDlgEditBox=GetDlgCtrlID(hwnd);

    if(idChildDlgEditBox==ID_STYLE_LIST)
    {
        FILE *File=NULL;
...
(HINSTANCE)GetWindowLongPtr(hwnd,GWLP_HINSTANCE),NULL);
            ...
            while(fgets(buf,1024,File))
            {
                ...
                szName=strtok(buf," ");
                if( ((c=strstr(buf,"WS_")) || (c=strstr(buf,"ES_"))) && c==buf)//ES car on traite les EDIT
                {
    ...
    }//fin if idChildDlgEditBox==ID_STYLE_LIST


    //Procedure du ID_ASPECT
    else if(idChildDlgEditBox==ID_ASPECT)
    {
    ...
    }//fin  if(idChildDlgEditBox==ID_ASPECT)
    else if(idChildDlgEditBox==ID_DESCRIPTION)
    {
